...
\end{mydef}
\loris{these definitions are not good, replace with following ones and add invariants}
\loris{--->}
\begin{mydef}
\label{def:fwdnew}
The relation $Fwd \subseteq S \times S \times PC $ captures the forwarding rule model. 
$(sw_1, sw_2, pc)\in Fwd$ if 
$sw_1$ forwards packets of class $pc$ to switch $sw_2$.
We write $Fwd(sw_1, sw_2, pc)$ for $(sw_1, sw_2, pc)\in Fwd$.
\end{mydef}
\begin{mydef}
\label{def:reachnew}
The relation $Reach \subseteq N \times PC \times K$ captures the reachability model \loris{I don't think model is the right word here}. 
$(sw, pc, k)\in Reach$ if 
the switch $sw$ is reachable in the path from source switch of packet class $pc$ in exactly $k$ steps.  
\end{mydef}
\loris{<---}
\loris{I would now state formally the invariants you are trying to accomplish.
e.g. Given two concrete relations Fwd and Reach the set $P$ of induced
paths is the following.
$sw_1\ldots sw_j \in P$ iff
...
}
...
\loris{with definition as relation you can write $\neg Fwd(sw_1,sw_2,pc)$ or  $(sw_1,sw_2,pc)\not\in Fwd$}
\begin{figure}[H]
...
In the section, we evaluate the number of constraints required to add to the z3 \loris{Z3 not z3, also put citation when you mention it for the first time} Solver. Let $|S|$ be the number of switches in the topology, $|L|$ be the number of edges in the topology, $|PC|$ be the number of packet classes, $|I|$ be the number of traffic isolation policies and $\mu$ be the max path length. Let $deg$ be the maximum number of neighbours for a switch (degree of the topology).
...
\subsubsection{Tactics Algorithm}
%
%\begin{mydef}
...
Incorporating tactics in the synthesis is sound. If the tactic imposes an restriction to the path, then the synthesis algorithm is incomplete. However, tactics can be used to specify restrictions which would be reasonably complete, thus speeding up the synthesis. 
