...
\section{Problem Statement}
\section{Network Forwarding Model} \label{sec:fwdmodel}
The key insights of the network forwarding model are \emph{two-fold}, abstracting the actual forwarding rules at each node, and encoding the reachability of each flow. \\ \\
$Fwd : N \times N \times PC  \rightarrow \{True,False\}$ is the forwarding rule model. 
$Reach : N \times PC \times L  \rightarrow \{True,False\}$ is the reachability model.
\begin{figure}[H]
	\includegraphics[width=8cm]{topoF.png}
	\caption{Network Forwarding Model}
\end{figure}
\subsection{Semantics} 
The $Fwd$ function encodes the actual forwarding rules at each switch. Therefore, $Fwd(sw_1, sw_2, pc)$ is set to \verb|true| means that $sw_1$ will forward packets of class $pc$ to 
%$F(s,d,pc,l) = True \Rightarrow$ For packet class $pc$ originating from $s$ can reach $d$ in atmost $l$ steps. \\ 
%For a flow from $<s1 \rightarrow d1>, F(s,n,pc,l) = True$, for all nodes $n$ in the path from $s1$ to $d1$, and $l$ encodes the length of the path. For example, in Figure 2, the path from $<1 \rightarrow 3>$ through waypoints \{4,5\} is $1 \rightarrow 4 \rightarrow 2 \rightarrow 5 \rightarrow 3$. \\ \\
%The path length of $1 \rightarrow 2$ is 2, so $F(1,2,pc, l \geq 2) = True$. \\ \\
%For a node $n1$ in the path which is not the source, we only encode the next hop of the node in F, $F(n1, n2, pc, l=1) = True$. We do not care about values of $l \ge 1$,  as all our reachability constraints are concerned from source $s1$.  

\subsection{Constraints}
\subsubsection{Forwarding Rule Constraints} 
...
\subsubsection{Flow Path Constraints}
%The semantics of our networking forwarding model for a flow with packet class $pc$ is concerned about path reachabilty from the source node $s1$ and next-hop reachability for two consecutive nodes in the graph. \\
%If $Reach(n,pc,k)$ is true, $Reach(n,pc,k + 1)$ is true as well because if flow $pc$ can reach $n$ in atmost $k$ links, it can reach $n$ in atmost $k+1$ links. 
%\[
%\forall n,k. Reach(n,pc, k)  \implies Reach(n,pc,k+1) \label{indpath} \tag{C5}
%\]

If a node $n_1$ is reachable in $k$, there must a node $n_2$ which is reachable in  $k-1$ steps and there exists a forwarding rule $n_2 \rightarrow n_1$.
\begin{multline}
...
\begin{itemize}
	...
	\[
	Reach(dst,pc,maxpathlen) = True \label{reach} \tag{C7}
	\]
	\item Waypoints :  $w \in W$ for flow $<s1 \rightarrow d1>$ : we need to assert the following.
	\[
	 \forall w \in W.Reach(w,pc, maxpathlen) = True \label{waypoint} \tag{C8}
	 \]
	 \item Traffic Isolation for \{$pc1,pc2$\} : We need to assert the following.\\
	 \[
	 \forall n_1. \neg ( \exists n_2. Fwd(n_1,n_2,pc1) \wedge Fwd(n_1,n_2,pc2)) \label{isolation} \tag{C9}
	 \]
\end{itemize}
...
\end{mydef}
The rationale behind the associated automaton $A^T$ is that the automaton accepts prefixes of the label-path, and rejects prefixes which cannot lead to a path satisfying the regular expression of the tactic (by reaching a sink state). 
...
%\[
%P \models T \iff  \Phi(P) \in L(A) 
%\]
%where $L(A)$ is the language of automaton $A$. 
%
% Thus, any path accepted by the tactic would a substring of a word in the language of the regular expression. The intuition is to prune the search to remove paths that will not satisfy the regular expression, that is paths which lead the automaton to the sink state.
...
\section{Conclusion}
 ...
 \begin{thebibliography}{1}
 	...
 	\bibitem{decentralize} O. Padon et. al
 	Shoham, Sharon {\em Decentralizing SDN Policies} 2015 
 	
 	\bibitem{netegg} Y. Yuan et. al {\em NetEgg: Programming Network Policies by Examples} 2014
 	
 	\bibitem{oneswitch} N. Kang et. al {\em Optimizing the "One Big Switch" Abstraction in Software-Defined Networks} 2013
 	
 	\bibitem{distfirewall} S Zhang et. al {\em An Adaptable Rule Placement for Software-Defined Networks} 2014
 	
 	\bibitem{netgen} S Saha et. al {\em NetGen: Synthesizing Data-plane Configurations for Network Policies} 2015
 	
 	\bibitem{z3} L De Moura et. al {\em Z3: An Efficient SMT Solver} 2008
 	
 	\bibitem{mcast} D. Ports et. al {\em Designing Distributed Systems Using Approximate Synchrony in Data Center Networks} 2015
 	
 	\bibitem{ply} PLY : {\em http://www.dabeaz.com/ply/}
 	
 	\bibitem{simple} Zafar Ayyub Qazi et. al {\em SIMPLE-fying middlebox policy enforcement using SDN} 2013
 	
 	\bibitem{metis} G Karypis et. al {\em A Fast and Highly Quality Multilevel Scheme for Partitioning Irregular Graphs} 1999 
