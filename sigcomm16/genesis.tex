...
\end{multline}
If $l_2 = c$, then \cref{eq:t1} will not work, as $l_1 .^i l2$ satisfies the tactic, and thus for switches with label $l_2=c,Reach(sw,pc,i+1)$ can hold. The correct modifications are as follows : 
\begin{multline} \label{eq:t1}
 \pi \vDash \neg (l_1 .^i c .^* l_2) \wedge c \not= l_2 \Leftrightarrow \\ \forall sw. \phi(sw) = c \implies \neg Reach(sw, pc, i + 1)
\end{multline}
