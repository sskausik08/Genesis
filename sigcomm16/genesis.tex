...
\begin{enumerate} 
	...
	\item \textbf{Traffic Isolation}:  $P1 \ || \ P2$ \\
	The traffic isolation policy ensures that the paths taken by packets $F1$ and $F2$, two reachability/waypoint policies do not traverse any link in the same direction. This policy can be used to provide fairness guarantees, since the paths of $P1$ and $P2$ don't share a link, the bandwidth used by $P1$ will not affect the bandwidth used by $P2$ and vice-versa. The condition of sharing a link in the same direction is due to the fact that links are full-duplex so, traffic flowing in one direction is not affected by the traffic flowing in the other direction.
	\item \textbf{Security Isolation}: $P1 <> P2$ \\
	The security isolation policy is stronger than the traffic isolation policy, and ensures that the path found for both the policies $P1$ and $P2$ do not share a link in both directions for security.
	...
	The switch table size policy is used to specify the size of the forwarding table of the 
\end{enumerate}
