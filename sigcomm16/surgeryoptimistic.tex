...
policies. \aditya{previous sentence does not parse} For packet classes
related by \aditya{is ``related by'' defined?} isolation policies, we
...
path obtained for $pc_1$ \aditya{this should be pc2?}. We term this
...
is a packet class for a reachability/waypoint policy \aditya{again the
  use of packet class and policy here is unconventional and needs to
  be carefully defined early on}. An edge $i \in I$ connecting
...
in the input specifications (like link capacity) \aditya{why? also,
  why is this focused on isolation?}. Given the policy graph $P$, we
...
We describe the heuristical optimistic synthesis algorithm in
\cref{optimisticsyn}. The crux of the algorithm is that we partition
the policy graph into two components $P_1$ and $P_2$ \aditya{are these
  ...
  incorrect}, synthesize $P_1$, and for packet classes of $P_2$
isolated to packet classes in $P_1$ \aditya{this seems incorrect: no
  such packet classes exist?}, add constraints to ensure that the
...
class. \aditya{it seems from the rest of the description below that p1
...
We use two schemes to partition the policy graph $P$ into two: the
first one is \emph{min-cut} partitioning, which partitions the graph
such that inter-partition graphs \aditya{edges?} are minimized. The
...
other packet classes. \aditya{do we repeat this recursively? if not, why not?}
...
synthesis. \aditya{need to be precise. what does ``to ensure that we
 ...
 when the number of solutions decreases and \aditya{is what follows
   after this a different observation, or related to the
   previous. confusing} it is tough to find a partial solution for
 ...
 and repeat. \aditya{when is this doubling done? during one of the
   recursive invocations, or only at the beginning?} By increasing the
 components' sizes, we can converge faster to the solution \aditya{I
