...
\item  $predicate$ is a packet header that identifies the packets pertaining to $r$ ;
...
For a reachability policy with ordered set of waypoints $s >> W_1;W_2 \ldots ... W_n >> d$ and packet class $pc$, we add all the constraints specified in \cref{sec:reach} to ensure the existence of a path from source to destination. To satisfy waypoint policies, we add constraints so that all waypoints $w$
are traversed. 
...
\end{equation}
We then add constraints stating that the size of the forwarding set must not exceed 1:\footnote{
	These constraints are expressed in SAT.}
\begin{equation}
...
All together these constraints ensure that all waypoints are traversed in the correct order. 
...
the constraints in \cref{eq:isolation} are still enough guarantee isolation.
...
\end{equation} 
For \emph{switch table size} policies, we use the relation $Reach$ to track the number of packet classes that traverses the switch.
We create inductive constraints similar to those for link capacity and use
them  to sum over the set of Boolean variables that denote reachability of the path to switch $sw$, which is $\exists k.~Reach(sw,pc,k)$.
\loris{this last sentence doesn't make sense, why the exists?}