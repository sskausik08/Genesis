...
Given a set of policies, we perform switch table synthesis by modeling the network forwarding model, and for each policy, adding a set of constraints to the SMT Solver Z3 \cite{z3}, such that the solution model satisfying the constraints can be used to extract the forwarding rules for the switches. 
\loris{previous sentence is too long.}
One of the key points of the network forwarding model is that the reachability, waypoint and isolation constraints are completely encoded using SAT i.e using boolean variables, while link capacity and switch table policies use the theory of integer linear arithmetic.
...
a tuple of the form $\{predicate$, $src$, $dst$, $W\}$ \loris{all tuples should be changed to round parenthesis} where
\begin{itemize}
\item  $predicate$ identifies the class of the packet and is drawn from a set \loris{??} of network headers;
...
\loris{why do we need this? we can enforce it with the length tactic now..}
...
\loris{text should be bigger and remove all the =True}
	...
	\label{fig:model}
\end{figure}
The network forwarding model is demonstrated by the example in \cref{fig:model}. 
...
these constraints act as the base case relating the $Fwd$ and $Reach$ relations. 
...
\loris{the previous point is a bit crucial, clarify a little better}
...
\begin{equation}
	\forall pc, k. \ \neg Reach(sw, pc, k)  
\end{equation}
\loris{this section is nonsense, we should just drop the link/node from the topology and changing the neighbouring function.}
