...
roughly translates to finding a solution in the solution space of size $n \times 242$.
\begin{figure}[H]
...
To describe high-level restrictions on the path, 
...
to create general search strategies which can be used for majority 
...
Also operators would not be comfortable writing policy specifications in terms of regular expressions 
...
but actually allow us to reduce the number of generated constraints. 
...
For example, $\neg (e .^i e .^* e)$ says that the path must not contain an edge switch at the $i+1^{th}$ step, unless it is the destination switch. 
...
We say that $\pi$ satisfies a tactic $R$, written $\pi\vDash R$, if the following
holds:
\begin{itemize}
\item $\pi \vDash \neg R$ iff $\pi \not\vDash R$;
...
constructs the path from destination to source. We use tactics to prune these constraints, such that
the path synthesized satisfies the tactic regular expression.  
We describe the tactic synthesis algorithm
for a path $\pi = sw_0 \ldots sw_k$ and labelling $\Phi(\pi) = a_0 \ldots a_k$. In the first
two cases, we remove some of the reachability prop. constraints (and adding unit negation clauses), 
...
\end{equation}
Thus, we can remove the reachability prop. constraints for 
the above mentioned tuples $(sw,pc,k) \notin Reach$
as they cannot construct the path satisfying the tactic.  

To satisfy tactics of the form $\neg (l_{src}  .^i l .^* l_{dst})$ where $l \not= l_{dst}$, we can similarly remove 
the reachability prop. constraints for these tuples:
...
In this case, we need to modify the reachability prop. constraints 
as follows which say that a switch with label $l$ cannot precede a switch (can only exist at the end of the path) : 
...
\end{multline}
To satisfy tactics of the form $\neg (l_{src}  .^i l_1 l_2 .^* l_{dst})$, the modifications are 
as follows to ensure that switch with label $l_2$ reached after $i + 2$ steps is not preceded by a switch of label $l_1$ : 
...
constraints which construct the path for a packet class. Each constraint says that if a switch is reachable in $k$ steps in a path,
 there must a neighbour switch in the path reachable at $k-1$ steps. We design the restricted tactic syntax based
  ...
  the constraints for  reachability in $i+3$ steps.
