...
\begin{figure}
	...
	\compactcaption{Overview of the system architecture of Genesis in a multi-tenant datacenter setting.}
...
\item \textbf{Middlebox traversals}. A tenant may wish that traffic
  ...
  group \loris{flow group is not defined, group of flows or set of flows?} to traverse, where the middleboxes in a set can be traversed
...
network
\loris{the previous sentence is too long}. For instance, to aid traffic enginerring, the network
...
middlebox placement and bandwidth constraints~\cite{} \loris{missing citation}. 
However these tools
...
\loris{maybe quick mention that this is the case for other tools: netgen}
...
as a set of constraints that describes why there wasn't a feasible solution~\cite{}.\loris{missing citation}
