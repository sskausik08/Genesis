...
to perform equi-sized partitioning.  The prototype contains ~5K lines
of Python code.  We plan to make the code for \Name publicly available.
...
two flows must be isolated on only certain links and can share other
links. This is an easier problem to tackle than isolation over all
...
path length is increased to 15 and no tactics are used (because it is
difficult to reason about the path satisfying a waypoint policy). The
...
unordered, so had ordered sets of sizes 1 and 2 in different
orders. \aditya{I don't follow this last sentence}
...
\begin{compactitemize}
	...
	complete tenant-isolation is in the range of 0.1-2000s. 
      \item Tactics provide a considerable improvement over the
        baseline, making \Name practical.  We can synthesize the above
        workloads in the range of 0.1-300s using the no edge tactic,
