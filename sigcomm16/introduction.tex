...
enterprise networks today. \aditya{give an example as to how slow
  things can get} To overcome this challenge, \Name  
...
\item a modular SMT-based algorithm for enforcing \Name policies that can easily be extended
			to novel types of policies \loris{we have some examples of these}
\item \loris{heuristics? I think the tactic based blacklisting should be one by itself here }
\item An implementation of \Name and its enforcing algorithm using the SMT
		solver Z3 together with an extensive evaluation on \loris{TODO}
\end{itemize}
\aditya{todo}
