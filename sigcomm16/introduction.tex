...
For many of the policies, automatically generating the controller is
an NP-complete problem and requires the design of efficient custom
heuristics; combining different policies' heuristics together is
non-trivial. \aditya{what does this previous sentence mean} \aditya{I
  had the following earlier. Why not go with that?  For many of the
...
\loris{I edited} By identifying a restricted syntax for specifying
...
required to ensure the solution satisfies the tactic. \aditya{quickly
 ...
 and degraded performance for only 20\% of the workloads.  \aditya{the
   previous sentence is vague} \loris{I'm not even sure what this
   is. If you are talking about dropping constraints when we don't
   have waypoints this is a pretty important contribution of the
   paper.  You have to explain that if some policies are not used we
   can further optimize the problem.  }  \kausik{Loris, do you want to
   mention that here?}  \aditya{I'm confused about Loris' question}
...
\item A modular SMT-based algorithm for enforcing \Name policies that can easily be extended
			to novel types of policies \loris{we have some examples of these} \kausik{apart from what we support?}
...
\loris{heuristics? I think the tactic based blacklisting should be one by itself here }
