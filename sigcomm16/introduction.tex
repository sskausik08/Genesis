...
languages focus on programming individual switch behaviors. \aditya{we
  ...
  switch-by-switch model} \kausik{Do you want to make changes here?}  There has been other research on
...
Unfortunately, naively encoding the problem using SMT solvers results in synthesis
...
enterprise networks today. \aditya{give an example as to how slow
  things can get} For instance, for synthesizing rules for a single reachability   
...
\kausik{Shouldn't we mention paths here instead of configurations? }
...
interactions to improve synthesis performance. The premise of this synthesis routine is to partition the input policies
 ...
 \aditya{the previous sentence is vague} 
\loris{I'm not even sure what this is. If you are talking about dropping constraints when
we don't have waypoints this is a pretty important contribution of the paper.
You have to explain that if some policies are not used we can further optimize the problem.
}  \kausik{Loris, do you want to mention that here?}
  \aditya{quickly talk about in 1-2 sentences the
