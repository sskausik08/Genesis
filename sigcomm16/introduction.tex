...
advent of cloud computing, these flow groups may be subject to
...
of rich policies to be specified in a simple and declarative way, 
so that the
complexities of correctly realizing the policies in the data plane are
hidden from the data center operators. This approach
paves the way for {\em intent-based networking}~\cite{}\loris{Kausik add missing citation}, where
...
enforcing complex policies.  We first show that enforcement of several of
...
The solution is then translated into switch rules. %% This
...
By leveraging the formal reasoning foundations of SMT solving,
\Name eliminates the room for error in the enforcement of even the
most complex collections of policies.\loris{not sure what this sentence is trying to say}
...
%is huge. 
For instance, even synthesizing 
rules that enforce
reachability among nodes in a 125 switch fat-tree topology can take
1s. This raises the need for better search strategies.  
To this
...
that are not desired in a data center network structure. 
\loris{how about: ...based on path patterns that are not desired...}
These tactics are used discard several constraints, therefore
...
separately and faster than the complete problem (due to the exponential time
 complexity). Across varying workloads, optimistic synthesis can
 halve the synthesis time for 40\% of the isolation workloads.  
 %\aditya{the
...
\item \loris{I really don't like this first sentence}
Tactics, a functionality for operators to specify blacklists on the properties of paths
 leveraging the hierarchical structure of datacenter topologies. 
 Tactics are specified using a restricted form of regular expressions and
 can vastly reduce the number of generated constraints.
