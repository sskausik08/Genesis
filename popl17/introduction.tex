...
deployments to cloud datacenters. In such environments, the different
entities (i.e., tenants) share different resources, such as, compute
...
multi-tenant datacenters thus have to deal with a multitude of
machines communicating with each other (flows) over a network that is
composed of many tens to hundreds of devices (routers or
...
communication among her applicaitons along shortest paths based on
certain metrics (reachability). In addition, she may wish that traffic
attempting to reach some of her applications be examined by a set of
``middleboxes'' for auditing and access control (traversal). Another
...
considerations.  In parallel, cloud operators must meet key
operational requirements. For instance, they often need to optimize
network traffic engineering objectives, e.g., minimizing the maximum
load imposed by all tenants on network links, and deal with
heterogeneous resource constraints such as switch table sizes. Also,
since datacenter networks are highly prone to link/switch
...
heuristics per policy type. Other recent works on network-wide policy
