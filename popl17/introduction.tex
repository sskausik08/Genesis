...
in complexity like isolation, middlebox
traversals, and traffic engineering 
Thus, \Name 
...
complex collections of policies. \Name can be easily extended 
to incorporate new policies over time.
...
a 1.5$\times - $400$\times$ speedup. 
 ...
 Secondly, \Name can use a \emph{divide-and-conquer}
 synthesis
...
\item An extensible declarative framework for describing
  ...
  like isolation, waypoints and traffic engineering (\Cref{sec:synthesisalgo});
\item A modified synthesis algorithm based on tactics, which leverages data center network structure
  to blacklist undesirable path patterns (\Cref{sec:tactic});
\item A divide-and-conquer synthesis procedure for speeding up synthesis by leveraging the 
structure of policy interactions (\Cref{sec:optimistic});
\item An implementation of \Name and an extensive evaluation on different policy workloads, topologies and multi-tenancy settings (\Cref{sec:evaluation}).
