...
parallel, cloud operators must meet key operational policies. For
...
With \emph{software-defined networking} (SDN),
...
heuristics {\em per policy type}. Other recent works on
...
We argue that \emph{data plane synthesis} can offer a practical
approach to realizing this vision in the multi-tenant datacenter
context.
%% switch
...
enforcing complex policies such as, isolation,
...
properties to simplify the constraints handled by the SMT solver.  We
allow the network operator to write restricted forms of regular
