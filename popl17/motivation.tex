...
\item \textbf{Isolation}. Tenants may require various QoS guarantees
 ...
 whose traffic will be isolated from one another, i.e.,
  the paths used by the tenants will not share any links in the topology. 
  A tenant could also specify isolation for a subset of her
  (performance-sensitive) flows from flows of the same tenant 
...
strict resource constraints, or optimize an objective like total utilization of the network
links. 
...
the red links are of low bandwidth, and the operator wants to ensure
...
%and $A2 \rightarrow C4$ do not exceed the link capacity of 100.
 ...
 When the current state of the network violates any policy, the 
 operator requires to find new forwarding rules such that the policies are
 satisfied. Naively performing this process can result in a solution
 which incurs a large overhead of installing the new rules (controller
 has to set up connections with switches and send the new rules). Thus,
 minimal network repair aims to minimize 
 the overhead of transitioning from an
 old data-plane to a new policy-compliant one. While repair
 is a reactive approach to failures, the response time to
 failures can be expensive. Instead, configuring switches proactively
 with resilient backup paths which comply with the policies 
 can reduce the time spent by the network in a state with policy violations,
 and operators can provide stricter guarantees during failures.
\end{compactitemize}


\noindent Today, realizing these policies requires painstaking,
often requiring manual configuration of network devices. This is incredibly
...
error prone. Even programming with SDN languages like 
Pyretic and Frenetic is challenging as 
these policies are global and cannot be
enforced by programming individual behavior of switches.
 While existing SDN-based
network management systems~\cite{simple,merlin,oneswitch} 
overcome these limitations, they are tailored
...
extensibility required to enforce the complex collection of policies
...
its only role is to install the forwarding rules to the switches.
...
objectives for each of the policies - ensuring isolation between paths
...
consider the space of all data planes and find a
...
handling a particular class of policy, but when multiple types of
policies are combined like isolation, waypoints, and traffic
...
of this complexity and allow us to unify search objectives for
...
techniques. To improve the synthesis performance,
 \name uses two techniques specific to policy enforcement
 to speed up synthesis: tactics (\secref{sec:tactic}) and 
