...
We introduce \emph{tactics} (inspired from the usage in SMT solvers, not 
proof assistants)
...
We now prove the soundness and completeness of the tactic synthesis algorithm. 
A path $\pi$ of the packet class $pc$ will be of the form $src \ldots dst$ due to the original synthesis
constraints. 
\begin{theorem}[Soundness]
	If the path $\pi$ for class $pc$ is a solution of the modified constraints, 
	then it satisfies the tactic regular expression.
\end{theorem}
\begin{proof}
The proof of this theorem is by contradiction for each type of tactic. \newline
\textbf{Type 1}:\\ $\forall sw,k \geq i + 1. (sw,pc,k) \notin Reach \implies \pi \models \neg (l_{src} .^i .^* l_{dst}) $ \newline
...
which is a contradiction, as $\exists sw, k \geq i + 1. (sw,pc,k) \in Reach$.
\newline
 \newline
\textbf{Type 2}: $(\forall sw.~ \phi(sw) = l ~\wedge~ sw \not= dst \implies  (sw, pc, i + 1) \notin Reach) \implies \pi \models \neg (l_{src} .^i \ l \ .^* l_{dst})$ \newline
...
which is a contradiction, as $\exists sw. ~ \phi(sw) = l ~\wedge~ sw \not= dst \wedge (sw,pc,i+1) \in Reach$.
 \newline \newline
\textbf{Type 3}: $(\forall n_1, n_2.~\phi(n_1) = l_1~\wedge~ \phi(n_2) = l_2 ~\wedge~ n_2 \not=dst  \implies \\ 
\neg (Reach(n_1, pc, i + 1) \wedge Fwd(n_1, n_2, pc)))\implies \pi \models \neg (l_{src} .^i \ l_1 \ l_2 \ .^* l_{dst})$ \newline
...
If a path $\pi$ is a solution of original constraints for class $pc$ and satisfies the tactic regular expression, then it is a possible solution of modified constraints.
\end{theorem}
\begin{proof}
The proof of this theorem is by contradiction for each type of tactic. \newline
\textbf{Type 1}:\\ $\pi \models \neg (l_{src} .^i .^* l_{dst}) \implies \forall sw,k \geq i + 1. (sw,pc,k) \notin Reach$ \newline
Lets us assume that $\pi$ is not a solution of the modified constraints $ \forall sw,k \geq i + 1. (sw,pc,k) \notin Reach$. \newline
...
Contradiction, as $\pi \models \neg (l_{src}\ .^i \ .^* \ l_{dst})$.
 \newline 
  \newline 
 \textbf{Type 2}: $\pi \models \neg (l_{src} .^i \ l \ .^* l_{dst}) \implies (\ \forall sw.~ \phi(sw) = l ~\wedge~ sw \not= dst \implies  (sw, pc, i + 1) \notin Reach)$ \newline
 Lets us assume that $\pi$ is not a solution of the modified constraints $\forall sw.~ \phi(sw) = l ~\wedge~ sw \not= dst \implies  (sw, pc, i + 1) \notin Reach$  \newline
 Therefore $\exists sw. \ sw \not= dst \ \wedge \ \phi(sw) = l \ \wedge \ (sw,pc,i+1) \in Reach$. \\
 Therefore, $\pi \models l_{src}\ .^i \ l \ .^* \ l_{dst}$ \\
 Contradiction, as $\pi \models \neg (l_{src} .^i \ l \ .^* l_{dst})$. 
  \newline  
  \newline
  \textbf{Type 3}: $\pi \models \neg (l_{src} .^i \ l_1 \ l_2 \ .^* l_{dst}) \implies (\ \forall n_1, n_2.~\phi(n_1) = l_1~\wedge~ \phi(n_2) = l_2 ~\wedge~ n_2 \not=dst  \implies  
  \neg (Reach(n_1, pc, i + 1) \wedge Fwd(n_1, n_2, pc)))$ \newline
   Lets us assume that $\pi$ is not a solution of the modified constraints $\forall n_1, n_2.~\phi(n_1) = l_1~\wedge~ \phi(n_2) = l_2 ~\wedge~ n_2 \not=dst  \implies \\ 
   \neg (Reach(n_1, pc, i + 1) \wedge Fwd(n_1, n_2, pc))$  \newline
