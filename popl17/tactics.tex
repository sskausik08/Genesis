...
Given a tactic $R_{pc}$ applied to packet class $pc$, 
we define $\Psi_T(R_{pc},pc)$ as the additional SMT constraints used for 
	synthesis. Note that $\Psi_T(R_{pc},pc)$ is presented as additional SMT 
constraints only for clarity. 
In practice, the modified synthesis algorithm will remove constraints 
for each packet class based on the tactic applied and provide
$(\Psi - \bigwedge\limits_{\forall pc} \Psi_T(R_{pc},pc))$ as input to the SMT solver. 
%In the first
...
\noindent\textbf{Type 1.} For a tactic $R_{pc}$ applied to $pc$ of the form $\neg (l_{src} .^i .^* l_{dst})$ which restricts the path to a length $ < i + 1$:
\begin{equation}
	\Psi_T(R_{pc}, pc) = ~ \forall sw,k \geq i + 1. (sw,pc,k) \notin Reach
\end{equation}
...
\noindent\textbf{Type 2.} For a tactic $R_{pc}$ applied to $pc$ 
of the form $\neg (l_{src}  .^i l .^* l_{dst})$:
\begin{multline} \label{eq:t1}
\Psi_T(R_{pc},pc) = \forall sw.~ \phi(sw) = l ~\wedge~ sw \not= dst \implies \\ 
(sw, pc, i + 1) \notin Reach
\end{multline}
...
\noindent\textbf{Type 3.} For a tactic $R_{pc}$ applied to $pc$ 
of the form $\neg (l_{src}  .^i l_1 l_2 .^* l_{dst})$: 
\begin{multline} \label{eq:t3}
\Psi_T(R_{pc},pc) = \forall n_1, n_2.~\phi(n_1) = l_1~\wedge~ \phi(n_2) = l_2 ~\wedge~ n_2 \not=dst \\  \implies 
...
%\Pi |= Psi_R.
Let $(Fwd, Reach)$ be a model of $\Psi$ and the set
of induced paths (from Definition \ref{def:Pi}), $\Pi = \texttt{paths}$ $(Fwd, Reach)$.
For each packet class $pc$, $R_{pc}$ is the tactic applied to the class. Note that the same
tactic or different tactics can be applied to all the classes. Let us denote the
set of tactics applied $\Gamma = \{(R_{pc},pc) ~| ~\forall pc\}$.
\begin{theorem}[Soundness]
	For a tactic set $\Gamma$, if $(Fwd, Reach) \models \Psi \wedge \bigwedge\limits_{(R_{pc}, pc) \in \Gamma} \Psi_T(R_{pc},pc)) $, 
	then $\forall (R_{pc}, pc) \in \Gamma. ~\forall(\pi', pc') \in \Pi. ~pc = pc' \implies \pi' \models R_{pc}$.
\end{theorem}

\begin{theorem}[Completeness]
For a tactic set $\Gamma$, if $~\Pi \models \Psi$ and 
$\forall (R_{pc}, pc) \in \Gamma. ~\forall(\pi', pc') \in \Pi. ~pc = pc' \implies \pi' \models R_{pc}$
then $\forall (R_{pc}, pc) \in \Gamma. (Fwd, Reach) \models \Psi_T(R_{pc}, pc)$.
\end{theorem}
