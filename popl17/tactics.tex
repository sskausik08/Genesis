...
Thus, $\pi = src\ sw_1 \ldots sw_i \ sw \ldots dst$ such that $\phi(sw) = l$. Also $sw \not=dst\\
Therefore,  $(sw, pc, i+1) \in Reach, \phi(sw) = l $. \\
However,
The modified constraints for this type of tactic are $\forall sw.~ \phi(sw) = l ~\wedge~ sw \not= dst \implies  (sw, pc, i + 1) \notin Reach$,
which is a contradiction, as $\exists sw, (sw,pc,k) \in Reach$. Hence, our assumption is incorrect. 
