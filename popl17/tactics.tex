...
We say that $\pi$ for packet class satisfies a tactic $(R, pc)$, written $\pi\models R$, if the following
holds:
...
\begin{theorem}[Soundness]
	For a tactic R, if $(Fwd, Reach) \models \Psi \wedge \Psi_R$, then $\forall (\pi, pc) \in \Pi. \ \pi \models R$.
\end{theorem}

\begin{theorem}[Completeness]
For a tactic R, if $~\Pi \models \Psi ~\wedge~$ $(\forall (\pi, pc) \in \Pi.~ \pi \models R)$, then $(Fwd, Reach) \models \Psi_R$.
\end{theorem}
