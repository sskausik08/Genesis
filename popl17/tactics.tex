...
We say that $\pi$ satisfies a tactic $R$, written $\pi\models_{pc} R$, if the following
holds:
\begin{compact2itemize}
\item $\pi \models_{pc} \neg R$ iff $\pi \not\models_{pc} R$;
\item $\pi \models_{pc}  l_{src} .^i .^* l_{dst}$ iff $k\geq i+1$, $l_{src}= a_0$, and $l_{dst}= a_k$; 
\item $\pi \models_{pc}  l_{src} .^i l.^* l_{dst}$ iff $k\geq i+2$, $l_{src}= a_0$, $l_{dst}= a_k$, and $a_{i+1}=l$;
\item $\pi \models_{pc}  l_{src} .^i l_1 l_2.^* l_{dst}$ iff $k\geq i+3$, $l_{src}= a_0$, $l_{dst}= a_k$, $a_{i+1}=l_1$, and $a_{i+2}=l_2$.
\end{compact2itemize}
...
\begin{mydef}
	...
	synthesis. 
\end{mydef}
We describe the additional SMT constraints $\Psi_R$ generated for all packet
classes
...
\begin{equation}
	\Psi_R = \forall pc~ \forall sw,k \geq i + 1. (sw,pc,k) \notin Reach
\end{equation}
...
\Psi_R = \forall pc~ \forall sw.~ \phi(sw) = l ~\wedge~ sw \not= dst \implies \\ 
(sw, pc, i + 1) \notin Reach
\end{multline}
...
\Psi_R = \forall pc~ \forall n_1, n_2.~\phi(n_1) = l_1~\wedge~ \phi(n_2) = l_2 ~\wedge~ n_2 \not=dst  \implies \\ 
...
%\Pi |= Psi_R.
Let $(Fwd, Reach)$ be the model of $\Psi$ and the set
