...
\subsubsection{Modified Synthesis Algorithm}
...
constraints only for clarity. The modified synthesis algorithm will provide 
...
\begin{theorem}[Soundness]
	For a packet class $pc$, if the path $\pi \models \Psi \wedge \Psi_R$, then $\pi \models R$.
\end{theorem}
\begin{proof}
...
However, $\pi \models \Psi_R \Leftrightarrow \pi \models\forall sw,k \geq i + 1. (sw,pc,k) \notin Reach$.
...
However,
$\pi \models \Psi_R \Leftrightarrow \pi \models\forall sw.~ \phi(sw) = l ~\wedge~ sw \not= dst \implies  (sw, pc, i + 1) \notin Reach$. \\
...
For a packet class $pc$, if the path $\pi \models \Psi ~\wedge~ \pi \models R$, then $\pi \models \Psi_R$ 
\end{theorem}
\begin{proof}
The proof of this theorem is by contradiction for each type of tactic. \newline
