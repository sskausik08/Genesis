...
One of the key challenges to synthesis performance is the number
...
mechanisms discussed in \secref{sec:recovery}.
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		...
		\State{attempts = 0} 
		\While{$attempts < RA_{max}$} \label{lst:line:solrecstart}
...
We use \emph{min-cut} partitioning to partition the policy graph
...
more likely to conflict with other packet classes. Our implementation
of \Name performs divide-and-conquer synthesis recursively on the
...
%to find a solution by solution recovery
...
% routine. 
 ...
 synthesis with varying isolation workloads in \secref{sec:optimisticeval}.
