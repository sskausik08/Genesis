...
\item What is the performance of Genesis with optimization objectives like traffic engineering
and network repair? (\cref{sec:optimizationeval})
...
\subsection{Divide-and-Conquer Synthesis Performance} \label{sec:optimisticeval} 
To evaluate the heuristic
divide-and-conquer synthesis procedure, we perform 100 runs of non-heuristic
...
speedup $t(non$-$dcsyn)/t(dcsyn)$ and plot its cumulative
	...
	\begin{footnotesize}
		\begin{center}
			\begin{tabular}{||P{7em} | P{13em} | P{4em} ||} 
				...
				\hline \hline \hline
				minimize-avg-te & 100 packet classes & 425 \\ [0.5ex]
				\hline 
				minimize-avg-te & 200 packet classes & 2002 \\ [0.5ex]
				\hline
				minimize-max-te & 50 packet classes & 4192 \\ [0.5ex]
				\hline
...
with different optimization objectives like traffic engineering and network repair. \name can
 ...
 for minimizing the max link utilization, \name can only synthesize 50 packet classes in ~4000 seconds(this is 
 a tough objective for the solver) and for both
 objectives, the synthesis time increases exponentially to increasing number of packet classes. SMT with
 optimization objectives is a emerging field of research, and we envision that solvers in the future will become fast
...
\newline
\noindent To summarize the key points of our evaluation are:
\begin{compactitemize}
