...
While policy-compliance is a major requirement in a network management
system, another important consideration is the ccurence of failures
(switches, links etc.), which are all too frequent in datacenter
networks~\cite{}. Failures require recomputation of paths compliant to
the policies for the modified topology.  A naive approach is to use
\name to resynthesize the modified instance; however, the new solution
may be drastically different from the original data plane, incurring a
large overhead of removing old rules and installing the new
...
gracefully. The first is data-plane resiliency.  This synthesizes and
pre-installs resilient data planes that, even in the event of a
...
capture certain objectives (Section XXX).

Thus, we propose a second \emph{minimal repair} mechanism (Section
XXX) which can transition with minimal overhead from the current data
plane to a policy-compliant new one by minimizing the number of
switches whose tables are modified.  Our minimal repair mechanism uses
MaxSMT. It does not have the memory cost of the first approach, and it
can capture all \Name policies and objectives. It is also useful to
accomodate incremental policy changes, which occurs frequently in
cloud datacenters~\cite{mpa-imc15}. Its main drawback is that it still
requires removal/installation of rules when a failure occurs, which
can still be expensive depending on the number of switches involved.
...
provide $t-resilience$~\cite{plinko}, i.e., in the event of upto $t$
arbitrary link failures, the synthesized data plane still has a path
for each packet class satisfying all policies which is achieved by
synthesizing backup paths that satisfy input policies.  We only
...
As an alternative to dataplane resiliency, we extend \name's synthesis
algorithm for performing minimal network repair using MaxSMT.
Formally, the MaxSMT problem is as follows: given a set of formulas
$\Psi_0, \Psi_1, \ldots \Psi_n$ with associated weights $w_1, \ldots
w_n$, find a subset $M \subseteq \{1, \ldots n\}$ s.t:
