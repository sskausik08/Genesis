...
we have
$SR(r_1, \lambda) = \{r_2\}$. Note that the 
inter-domain static routes we place
for a domain assignment $\Theta$ is necessary, 
finding optimal static routes for intra-domain  
is hard. 
\kausik{Done: clarify that hard part of static routes is intra domain
	and the ones we place for inter are necessary}
