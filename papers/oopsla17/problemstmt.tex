...
One of the main considerations in this paper is to 
generate configurations that are highly resilient to failures.
For a packet class $pc$ and configuration $C$, we define 
the set of \emph{affecting links} $\links(pc)$ as follows: 
$\links(pc) = \{l \mid l \in L.~\exists \pi \in \paths^C(pc).~l \in \pi  \}$.
For a single link $l$ failure, a packet class $pc$ is affected 
only if $l \in \links(pc)$. 
We present two different resilience metrics that we will use in the following sections. 
...
is defined as follows:
\[
\frac{\sum_{\forall pc} |\{ l \mid l \in \links(pc) ~\wedge~ ((V, L\setminus \{l\}),\Theta,W,LP,IF,SR) \models (\Psi_{pc}, P) \}|}
{\sum_{\forall pc} |\links(pc)|}
\]
where $C \models (\Psi_{pc}, P)$ denotes that $C$ is 
compliant for policies $\Psi_{pc}$ which affect packet class $pc$.

\loris{ugly name, open to suggestions} 
...
is defined as follows:
\[
\frac{\sum_{\forall pc} |\{ l \mid l \in \links(pc) ~\wedge~ 
	C'= ((V, L\setminus \{l\}),\Theta,W,LP,IF,SR) ~\wedge~ 
	\paths^{C'}(pc) \not= \emptyset  \}|}
{\sum_{\forall pc} |\links(pc)|}
\]
