...
% \kausik{Assume that in domain $d$, traffic 
...
Given a configuration $C$, the set of induced paths $\paths^C(pc)$
by the configuration $C$ for the packet class $pc$ is defined as follows:
\begin{multline*}
\paths^C(pc) = \{\pi=l_1\ldots l_k ~|~ valid(\pi) ~\wedge ~l_1 = (s_{pc}, v_1) \in \pi  ~\wedge ~l_k=(u_k, d_{pc}) ~\wedge \\
~\forall ~l_i=(u_i, v_i) \in \pi. ~v_i \in \route^C(u_i,\lambda_{pc})\}
\end{multline*}
Given a configuration $C$ and a set of packet classes $PC$, the set of paths
$\paths^C(PC)$ induced by $C$ is defined as: 
\[
\paths^C(PC) = \bigcup_{\forall pc} \paths(pc)
\]
\end{definition}
...
\kausik{1) route filters, which in general reduce the resilience of the network---i.e., $rc=|\cup_{\lambda\in \Lambda} RF(\lambda)|\leq C_{rc}$},
1) static routes which can lead to undesired routing behaviors under failures like routing loops (\secref{sec:loopavoidance})---i.e., $sc=|\{(v, v', \lambda)\mid \exists v'. v' \in SR(v,\lambda)\}|\leq C_{sc}$, and
