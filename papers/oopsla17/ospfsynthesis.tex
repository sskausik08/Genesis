	...
	\subfloat[Edge Weights]{
		\raisebox{0.5cm}{\resizebox {0.33\columnwidth} {!} {
			...
			\subfloat[Routing Loop]{
				\raisebox{0.6cm}{\resizebox {0.33\columnwidth} {!} {
				\begin{tikzpicture}[shorten >=0.5pt,node distance=,on grid,auto,
					square/.style={regular polygon,regular polygon sides=4}] 
					\node[state] at (0,0) (s)  {$s$}; 
					\node[state] at (1.8,1) (v1)  {$r_1$}; 
				...
				\end{tikzpicture}
				 ...
				 such that the routers forward traffic along
					the colored paths and routing loops potentially caused
					by static routes.}
...
the sum of the weights belonging to the path from $s$ to $R_\lambda$ in $\xi_\lambda$ 
...
to a next-hop not on the OSPF shortest path, we can 
...
but the path through $r$ is creating the infeasibility. 
