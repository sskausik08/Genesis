...
\todo{use following text to argue about static route bad for resilience}
...
the static route into account (\secref{sec:loopavoidance}). 
...
\todo{need to argue that minimizing static routes is the right thing to do}
...
\todo{reiterate that going straight to policies is hard
so instead we do path first and then look for path-compliant configs}
...
We first show how to solve when there exists a solution with no static routes
and then
