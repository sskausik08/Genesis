...
Traditional hierarchical networks are partitioned primarily
based on ownership of domains--- each autonomous system 
is owned by a separate entity, or due to hardware 
limitations--- the intra-domain routers  
may not be equipped to run BGP. 
Many networks today have well-defined routing hierarchies,
but these are realized through careful painstaking
engineering. For example, it is often difficult to determine
which routers to include in an OSPF area, how many OSPF
domains to create, etc. Enterprise and datacenter 
networks offer flexibility in routing protocols run on each
router (BGP/OSPF)
and mechanisms to update domain assignments periodically.
In such settings, \name can automatically find
good routing domain hierarchies with increased
resilience/decreased complexity. We envision that operators
will synthesize ``one-shot'' domain assignments for their changing 
input policies at coarse-grained timescales (days/weeks);
operators need not synthesize these assignments for every 
policy change. 
