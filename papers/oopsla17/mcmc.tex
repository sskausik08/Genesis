...
connectivity-resilience. Instead of directly reasoning about 
resilience which is hard, we try to minimize the number of static routes, 
which leads to greater connectivity-resilience scores.
...
the problem generating a domain assignment for which
there exists a 
configuration that path-compliant  with $\Pi$ with no static routes
is NP-complete.
\end{theorem}
\iffull
