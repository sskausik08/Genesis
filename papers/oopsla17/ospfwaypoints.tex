...
\begin{wrapfigure}{r}{0.38\columnwidth}
		\vspace{-2mm}
			...
			\end{tikzpicture}
			\compactcaption{Waypoint-compliant edge weights.} 
...
$r$. For example, 
\todo{broken ref}
in \Cref{fig:ospfexample}(c), there is a routing loop because
...
Since the paths are disjoints, if a single link fails, at most one 
...
correspond to the ones with shorter, thus simply 
...
non-shortest path. 
Using this property we can  estimate 
\todo{over or underapproximate?}
the
