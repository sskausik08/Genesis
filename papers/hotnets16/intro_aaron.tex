...
Unfortunately, as we show in \secref{sec:TODO}, synthesizing a control plane
directly from policies does not scale due to \aaron{need a pithy summary}.
Synthesizing from examples, while scalable, requires many example paths to get
the control plane right (\secref{sec:TODO}). Generating example paths is hard,
which is part of the reason we want to synthesize the control plane!
\loris{I don't get this part on the synthesis from examples. In synthesis from examples
one typically generalizes from examples that are ground truth and express correct
behaviour.}
...
Instead, we propose a two-phased approach (\secref{sec:synthesis}). First, we
