...
\Cref{fig:process} can be implemented.
First, we briefly discuss how existing tools can be used to generate
policy-compliant paths (\secref{sec:phase1}).
Second, we show how to generate ARCs that induce
the set of paths synthesized in the first phase (\secref{sec:phase2}).
Third, we show how ARCs can be modified to deal with resilience (\secref{sec:phase3}).
We then show how to restart the process whenever one of the three phases fails (\secref{sec:restart}).
Finally, we show how we can transform ARCs to actual device configurations (\secref{sec:deviceconf}). 
