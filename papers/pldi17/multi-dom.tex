...
\loris{again, I say route filters}
In this setting, we the policies in $P$ will try to minimize some expression
over the number
route filters, static routes, and BGP configurations.
First, our algorithm determines \loris{the optimal?}
...
to determine the edge weights and route filters in each domain to guarantee that paths traverse
...
\begin{figure*}
	\centering
	\subfloat[Single
	BGP Gateway]{\includegraphics[width=0.66\columnwidth]{figures/bgp-example.pdf}}
	\hfill
	\subfloat[Multiple BGP Gateways]{\includegraphics[width=0.66\columnwidth]{figures/bgp-example2.pdf}}
	\hfill
	\subfloat[Static Routes]{\includegraphics[width=0.53\columnwidth]{figures/static-route.pdf}}
...
\loris{ CHANGE dst to lambda in fig.}
...
path (1) to be the chosen path from source $src$\loris{check if src is called src after fixing fig} to destination $\lambda$.
...
Instead, $g_1$ and $g_2$ receive advertisements for a route $2 \rightarrow 1$\loris{this arrow notation used consistently?}
...
100, $LP(??, ??, \lambda)$\loris{put correct values},
which causes $g_1$ to become gateway at domain 0 for destination $\lambda$, i.e., 
$G^C(0, r_1, \lambda) = g_1$.

The third case is when two paths from different sources in the same domain, but for the same destination $\lambda$, exit 
...
we add an iBGP filter\loris{is this like a route filter, our model doesn't have it?} for the connection $g_2 \rightarrow g_1$ for
...
algorithm from \secref{sec:intra-synthesis} to assign OSPF weights to the links in the domain
