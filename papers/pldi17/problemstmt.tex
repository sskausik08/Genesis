...
We also define $\in$ operator for paths i.e., a link $l \in
...
Given a BGP router $r$, neighbouring router $r_1$,
and IP destination $\lambda$,
  $LP(r, r_1, \lambda)$ 
specifies the local preference for a route received 
for destination $\lambda$ at BGP router $r$ 
from next-hop BGP router $r_1$. 
This function is defined on inter-domain links. 
...
We now describe how routing happens across domains.
...
The first type of policy restricts how many OSPF domains $N_D$
a configuration might have, $c_1\leq N_D\leq c_2$, \loris{update table}
and it is satisfied by $C$ iff $c_1\leq |\{\Theta(r)\mid r\in V\}|\leq c_2$.
This policy is useful for avoiding situation in which too many domains
can cause flooding 
of BGP updates and  
overwhelm the network when links fail. 
For the same reason, the second type of policy allows the operator to restrict the size $k$ of each OSPF domain,
$l\leq k\leq u$.
This policy is satisfied if  for every domain $i$, $l\leq |\{r \mid \Theta(r)=i\}|\leq u$.
...
\name also allows to minimize certain expression that the quantities $rc$, $sc$, and $bc$---e.g., $max(rc, sc+bc)$.
...
\paragraph{Synthesis problems}
 We are now ready to define our synthesis problems.
...
and a set of paths $\Pi$ over packet classes $PC$,
	a configurations $C$ is path-compliant with respect to 
	$(\Pi,P)$,
