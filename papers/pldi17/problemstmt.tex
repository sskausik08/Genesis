...
%	~~~~~~~~~~~~~~~~~~~~~~~~~\route_{ospf}^C(s_2,g_2, \lambda) + (g_2, s_3) + \\
	\ldots  + (g_{n-1}, s_n) +\route_{ospf}^C(s_n,t,\lambda)
\end{multline}
...
\name also allows to minimize certain expressions over the quantities $rc$, $sc$, and $bc$---e.g., $max(rc, sc+bc)$. Ideally, we would like to 
increase the resilience of the endpoints (in terms of alternate
paths available), however it is harder to reason about. Wherever
possible, we try to maximize resilience, otherwise, we try to
minimize number of filters to increase resiliency. 
