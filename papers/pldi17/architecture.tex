...
We modify the constraints given to \genesis to ensure that,
...
If we allow pats of length at most $\mu$, we can define $Reach^*(r,pc)$ as:
\begin{equation}
	Reach^*(r,pc) = \bigvee_{k \in [0, \mu]} Reach(r, pc, k).
\end{equation}
To guarantee that the generate paths can be enforced using destination-based
forwarding, for every pair of packet classes $pc_1$ and $pc_2$ that belong to the same 
 destination subnet, we add
the constraint:
 \begin{multline}
