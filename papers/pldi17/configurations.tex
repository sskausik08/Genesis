...
a topology $T=(V,L)$,
\loris{configuration policies $P$,}
...
\loris{I've been linking to table 1 instead of table 2 everywhere, please fix}
...
the number of route filers \loris{??},
the number of static routes $sc$,
and the number of BGP configurations entries $bc$.
Computing the cost $c(\Theta,e)$ amounts to substituting in $e$ the values of \loris{??},
...
However, we showed that, given a domain assignment, computing the  minimal required number of route-filters \loris{??}
...
\name estimates the number \loris{??} of required route-filters by counting for how many $(r_i, r_j, \lambda, \lambda')$-diamonds
$\Theta(r_i) = \Theta(r_j)$. 
...
produce reduced number of filters (\Cref{some experiment}). 
