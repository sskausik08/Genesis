...
\minisection{OSPF} Open Shortest Path First (OSPF) routing
is commonly used inside a domain. In OSPF, each link 
is associated with a positive weight and traffic from a router $r$ to
...
is directly connected to a single router $r_\lambda$, 
for all routers $r$ in the 
same domain as $r_\lambda$,
if $\Theta(r) = \Theta(r_\lambda)$, 
we have that $G(\Theta(r), \lambda) = \{r_\lambda\}$.
...
preference for the router $r$ to choose $r'$ 
as the next-hop router for destination $\lambda$.  
...
Therefore, if $r \in G(\theta, \lambda)$,
BGP is preferred and $\route^C(r, \lambda)=\route^C_{bgp}(r, \lambda)$. 

Finally, for a router $r$ in $\theta$ 
which is not a BGP gateway 
router for $\lambda$ 
and has no static routes, 
$r$ uses OSPF to route traffic to the closest BGP
gateway router in $G (\theta, \lambda)$, 
which in turn will forward  the traffic to its 
neighbouring domains and so on till it reaches the 
internal domain of $\lambda$.  
\iffull
Thus, the modified OSPF routing 
...
protocols, and thus, operators require
 support to 
 enforce and/or optimize different
configuration  structure and properties. We 
classify
 Zeppelin's 
 policy support  into two categories: 
...
can lead to undesired routing behaviors such as routing loops (more in~\secref{sec:loopavoidance}). Similarly, it is desirable to limit the number of BGP configurations
