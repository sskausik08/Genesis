...
any guarantees as to whether the policies are complied when
...
\begin{figure}[!t] 
	...
	\subfloat[Waypoint-compliant edge weights]{
		\resizebox {0.4\columnwidth} {!} {
			...
			\node[state, fill=orange] at (2,1) (v1)  {$r_1$}; 
			\node[state, fill=orange] at (2,-1) (v2)  {$r_2$}; 
			\node[state] at (4, 0)(t) {$t$};
			\node[state, rectangle] at (5.5, 0) (d1) {$\lambda$};
			\path[->] 
		...
		\subfloat[Routing Loop]{
			\raisebox{0.7cm}{\resizebox {0.4\columnwidth} {!} {
					\begin{tikzpicture}[shorten >=0.5pt,node distance=,on grid,auto,
					square/.style={regular polygon,regular polygon sides=4}] 
					\node[state] at (0,0) (s)  {$s$}; 
				...
				\caption{Example of waypoint-compliant edge weights 
...
\resizebox {0.6\columnwidth} {!} {
	\begin{tikzpicture}[shorten >=0.5pt,node distance=,on grid,auto,
	square/.style={regular polygon,regular polygon sides=4}] 
	\node[state] at (-0.5,1.2) (v3)  {$r_3$}; 
	\node[state] at (-2,0) (s)  {$r_0$}; 
	\node[state] at (-4,0) (src)  {$s$}; 
	\node[state, fill=orange] at (1,0) (v1)  {$r_1$}; 
	\node[state, fill=orange] at (-0.5,-1.2) (v2)  {$r_2$}; 
	\node[state] at (3, 0)(t) {$t$};
	\node[state, rectangle] at (4.5, 0) (d1) {$\lambda$};
	\path[->] 
	...
	(t) edge [red, dashed] node {} (d1);
	\end{tikzpicture}}
	\caption{Example non-resilient configuration for the red paths provided by Genesis for $\waypt = \{r_1, r_2\}$. If link $r_1 \rightarrow t$ 
