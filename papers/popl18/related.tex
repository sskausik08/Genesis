...
domain decomposition and inter-domain routing. \aditya{check this}
...
but it does not
support any notion of policy- or connectivity-resilience  \aditya{check this}
...
complexity, or OSPF, or to express other policies such as
isolation. \aditya{check the last part}
...
LP-solvers---which are faster and parallelizable---to tackle
configuration synthesis rather than SMT solvers.  Synet's approach
does not deal with resilience, a key aspect we tackle in this paper.
Synet also does not attempt to minimize the number of static routes,
