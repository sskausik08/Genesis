...
However, since \Cref{eq:waypoint} does not 
\iffull
\begin{figure}[t]
\else
...
a waypoint-compliant path, $\pi_2$, the configuration will not switch to it.
%but ``worse'' than
...
equate them to split the traffic.
 ...
 OSPF distance can be estimated by sum of edge weights,
 while the distance from $r_0$ to $t$ is estimated using $D(r_0, t)$.
 Concretely, given $\pi_1 = (s, r_1)(r_1, r_2) \ldots (r_m, t)$, let 
 $r_\alpha$ denote the first router in the path which has a static route---
 i.e., $\forall i < \alpha. (r_i, r_{i+1}) \not\in SR(\lambda)$ for destination IP $\lambda$.
 we estimate
...
$\pi_1$ and $\pi_2 = (s, s_1)(s_1, s_2)\ldots(s_n, t)$, we ensure that 
estimated OSPF distance of $\pi_2$ (with first static route at $s_\beta$)
...
\end{equation}
However, distance variables $D$ 
are upper bounded by actual
distances and may not be equal, thus, the term $D(r_\beta,t)$ 
will not faithfully represent the distance here as the constraint is simply 
imposing an upper bound on an upper bounded variable. 
Thus,  we use $\sum_{\pi_2}W$ (which is an upper bound)
