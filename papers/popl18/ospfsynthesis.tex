...
\begin{proof}
	...
	shortest path between $s$ and $d$. Let  $\overline{\pi} = (s, r_1)(r_1, r_2)\ldots (r_m,
	 d)$ be one of the shortest path that is different from $\pi$. 
	 \loris{just assume $s_i \neq r_i$ is the first point of divergence and change rest of proof.
	However, you need to modify eq 4 to say that you do also for things that are not $r_1$,
	or at least say it in the text. Right now eq 4 really only works for $r_1$.
	 }
    Without loss of generality, we assume $s_1 \not= r_1$, 
	else, we can consider the source to be 
	the first switch where the paths diverge. 
	
	\noindent Since, $\overline{\pi}$ is the shortest weighted path: 
	\begin{equation} \label{eq:assumption}
	\sum_{\pi} W(s_i, s_j) \geq \sum_{\overline{\pi}} W(r_i, r_j)
	\end{equation}
	
	Constraints (\ref{eq:unique}) impose $\pi$ to be the
	unique shortest path by ensuring the edge weights of $\pi$
	are smaller than any path going through a neighbouring 
	router of $s$ other than $s_1$. 
	\loris{you switched notation: $W_s^t$ vs $W(s,t)$	}
	Consider the 
	neighbour router $r_1$ in $\overline{\pi}$:
   \loris{you mean $W(s_i,s_{i+1}) < W(s, r_1)+ W(r_1, r_2) + D(r_2, d)$?
   Also the notation $\sum_\pi$ is never used before.}
	\[
	\sum_{\mathclap{\substack{\pi}}} 
	W(s_i,s_j) < W(s, r_1)+ D(r_1, d)
	\]
	We can then use constraint (\ref{eq:distance}) to expand the term $D(r_k, d)$ for $k \leq m$
	and obtain the following: 
	\[
	\sum_{\mathclap{\substack{\pi}}} 
	W(s_i,s_j) < W(s, r_1)+ W(r_1, r_2) + D(r_2, d)
	\] 
	\begin{center}
		$\ldots$
	\end{center}
	\[
	\sum_{\mathclap{\substack{\pi}}} 
	W(s_i,s_j) < W(s, r_1)+ W(r_1, r_2) + \ldots W(r_{m}, d) + D(d, d)
	\] 
	\[
	\sum_{\mathclap{\substack{\pi}}} 
	W(s_i,s_j) < 
	\sum_{\mathclap{\substack{\overline{\pi}}}} 
	W(r_i,r_j)
	\]
	This contradicts assumption (\ref{eq:assumption}) that 
	$\overline{\pi}$ is one of the shortest paths from $s$ to $d$. 
		...
		\begin{algorithm}[H]
			\begin{footnotesize} 
				...
				\begin{algorithmic}[1]
					...
					\State{Pick static route $sr(s, t, \lambda)$ from $uc$ constraints}
					\State{Add $sr$ to static routes $SR$}
					\State{$\Psi = \Psi \setminus \Psi_U(s, \lambda)$}
					\EndWhile
