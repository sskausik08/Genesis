...
\begin{proof}
	...
	= (s, s_1)(s_1, s_2) 
	\ldots (s_n, d)$ in $Pi$ such that $\pi$ is \emph{not} the shortest 
	path between $s$ and $d$. Let us denote the shortest
	 path by $\overline{\pi} = (s, r_1)(r_1, r_2)\ldots (r_m,
	 d)$. 
	...
	\sum_{\pi} W(s_i, s_j) > \sum_{\overline{\pi}} W(r_i, r_j)
	\end{equation}
	
	The constraints (\ref{eq:unique}) impose $\pi$ to be the
	unique shortest path by ensuring the edge weights of $\pi$
	are smaller than any path going through a neighbouring 
	router of $s$ other than $s_1$. Let us consider the 
	neighbour router $r_1$ in $\overline{\pi}$:
	\[
	\sum_{\mathclap{\substack{\pi}}} 
	W(s_i,s_j) < W(s, r_1)+ D(r_1, d)
	\]
	Using constraint (\ref{eq:distance}) to expand the term $D(r_k, d)$ for $k <= m$: 
	\[
	\sum_{\mathclap{\substack{\pi}}} 
	...
	$\overline{\pi}$ is the shortest path from $s$ to $d$. 
	Hence, proved.
\end{proof}
	...
	\begin{minipage}{\linewidth}
		\begin{algorithm}[H]
			\begin{footnotesize} 
				...
				\label{alg:unsat}
				\begin{algorithmic}[1]
					\Procedure{OSPFSynthesis}{} 
					\State{$\Psi:$ Constraints generated from (\ref{eq:distance}) and (\ref{eq:unique})} 
					\While{$\Psi$ is \emph{unsat}} 
					\State{Extract unsat core $C$ from LP Solver}
					\State{Pick one static route $sr$ from $C$ constraints}
					\State{Add $sr$ to static routes $SR$}
					\State{$\Psi = \Psi \setminus C$}
					\EndWhile 
					\EndProcedure
				\end{algorithmic}
			\end{footnotesize}
