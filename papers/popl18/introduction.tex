...
applications' diverse security and compliance requirements. Second,
with increasing sizes of networks, it must provide support for
realizing hierarchical control planes -- where a network is split into
several ``domains'' atop which a hierarchy of control plane
configurations is deployed -- to ensure scalability, necessitating the
use of multiple intra- and inter-domain routing protocols (e.g., OSPF
and BGP). Third, to improve manageability and network
...
routers. Finally, and most importantly, it must ensure that
configurations are resilient to network malfunctions such as link
failures.

\aditya{edited till here. will get back in a couple hours}
