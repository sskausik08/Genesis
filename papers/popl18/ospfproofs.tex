...
\begin{theorem}[Correctness]
	Given a set of paths $\Pi$, \Cref{alg:unsat} outputs
	a configuration $C(W,SR)$ such that, every path in $\Pi$ is
	induced by the network configuration $C$.
	\loris{why not "such that $\Pi\subseteq\paths^C(pc)$"?}
...
$W$ (\secref{sec:routingmodel}), let the first router where routing diverges from $GP$ be $s_p$---i.e.,  
$s_{p+1} \not\in \route^C(s_{p}, \lambda_{pc})$. Let us consider  $r \in \route^C(s_{p}, \lambda_{pc})$.  
\Cref{alg:unsat} on line \loris{??} adds the following \loris{is the following (4) or (6)? Reword}
...
\loris{in the following, refer to line numbers to help the reader understand}
